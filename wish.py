...
def roll(wish, banner5, standard5, banner4, standard4, fifty_fifty5, fifty_fifty4, garbage, soft_pity, count):
    if wish <= 6 + soft_pity:
        banner5, standard5, fifty_fifty5 = characterRoll(banner5, standard5, fifty_fifty5)
        #print(f"You rolled a 5-star at {count%90}") # Writes how deep into pity you was while pulling a 5-star (if you do a lot of wishes i reccomend to comment it because there'll be a lot of printing...)
    elif wish <= 51 + soft_pity and wish > 6 + soft_pity:
        banner4, standard4, fifty_fifty4 = characterRoll(banner4, standard4, fifty_fifty4)
    else:
        garbage += 1
    return banner5, standard5, banner4, standard4, fifty_fifty5, fifty_fifty4, garbage
...
def main():
    ...
    pity = 1
    soft_pity = 0 # Soft pity is a theory popular among Genshin players. It postulate that if you reach 75 pity, chances to drop 5-star (slightly) increase. Most of 5-star pulls occur between 75 to 85 wishes, so it seems like a reasonable theory, but it was never confirmed by the devs.
    ...
    for wishes in range(num_of_wishes):
        if pity % 90 == 0 and pity != 0:
            ...
            #print("you rolled guaranteed 5-star") 
            pity = 0
            ...
            banner_five_star_count, standard_five_star_count, banner_four_star_count, standard_four_star_count, five_star_fifty_fifty, four_star_fifty_fifty, garbage_count = roll(wish, banner_five_star_count, standard_five_star_count, banner_four_star_count, standard_four_star_count, five_star_fifty_fifty, four_star_fifty_fifty, garbage_count, soft_pity, wishes)
        pity += 1
